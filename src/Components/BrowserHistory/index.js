...
class BrowseHistory extends Component {
  ...
  enterTextToSearch = event => {
    this.setState({searchInput: event.target.value})
  }

  deleteHistoryItem = id => {
    const {historyList} = this.state
    const remainItems = historyList.filter(eachItem => eachItem.id !== id)
    if (remainItems.length === 0) {
      this.setState({historyList: remainItems, isTrue: true})
    } else {
      this.setState({historyList: remainItems})
    }
  }
          ...
          <div className="search-input-container">
            ...
            <div className="input-container">
              <input
                ...
                onChange={this.enterTextToSearch}
                type="search"
                value={searchInput}
              />
            </div>
        ...
        {!isTrue && (
          <ul className="history-items-card">
            {searchResults.map(item => (
              <HistoryItem
                itemToDelete={this.deleteHistoryItem}
                item={item}
                key={item.id}
              />
            ))}
